    ...
    <Modal open={open} onClose={onClose} title="Новый оператор">
      <div className="flex flex-col gap-4">
        ...
        <div className="flex items-center justify-between p-3 rounded-lg border border-border bg-muted/20">
          ...
          <button
            type="button"
            onClick={() => setForm(p => ({ ...p, canReceiveInbound: !p.canReceiveInbound }))}
            className={['relative inline-flex h-6 w-11 rounded-full transition-colors', form.canReceiveInbound ? 'bg-primary' : 'bg-muted'].join(' ')}
          >
            <span className={['absolute top-0.5 left-0.5 w-5 h-5 rounded-full bg-white transition-transform shadow', form.canReceiveInbound ? 'translate-x-5' : 'translate-x-0'].join(' ')} />
          </button>
        </div>
