...
export type AdminCommand =
  ...
  | { type: 'acw:force-close' }
  | { type: 'call-connected'; payload: { callId: number; phone: string; numberData?: Record<string, any> | null } };
...
export function useAdminSocket({ operatorId, onCommand, onReconnect }: UseAdminSocketOptions) {
  ...
  useEffect(() => {
    ...
    // On HTTPS (nginx): use same origin (port 443) — nginx proxies /socket.io/ → :3001
    // On HTTP (dev): connect directly to backend port 3001
