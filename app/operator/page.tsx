...
export default function OperatorPage() {
  ...
  const [currentCall, setCurrentCall] = useState<{ phone: string; callId?: number } | null>(null);
  ...
  const syncStatusToDB = useCallback((s: OperatorStatus, pr: PauseReason | null) => {
    if (!user?.id) return;
    // Don't persist transient call states — store IDLE instead
    const persisted = (s === 'DIALING' || s === 'TALKING' || s === 'ACW') ? 'IDLE' : s;
  ...
  const { connected: wsConnected } = useAdminSocket({
    ...
    onReconnect: () => {
      // Re-register availability on reconnect
      if (user?.id) operatorsApi.setAvailable(user.id, status === 'IDLE').catch(() => {});
      // Show banner so operator knows server was restarted
    ...
    onCommand: (cmd) => {
      if (cmd.type === 'kick') {
        ...
        const { callId, phone } = cmd.payload ?? {};
        if (callId) {
          setCurrentCall(prev => prev
            ? { ...prev, callId, ...(phone ? { phone } : {}) }
            : { phone, callId });
        }
      }
    },
  ...
  useEffect(() => {
    ...
    // Restore pause timer from DB timestamp
    if (restored === 'PAUSE' && user.pauseStartedAt) {
      const elapsed = Math.floor((Date.now() - new Date(user.pauseStartedAt).getTime()) / 1000);
      setPauseInitialSec(Math.max(0, elapsed));
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [user?.id]);
  ...
  useEffect(() => {
    if (sip.status === 'calling' && sip.incomingCall) {
      ...
      const remote = sip.incomingCall.remote_identity?.uri?.user ?? 'Неизвестный';
      setCurrentCall({ phone: remote });
  ...
  useEffect(() => {
    if (sip.status === 'registered' && (status === 'TALKING' || status === 'DIALING')) {
      setStatus('ACW');
    }
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [sip.status]);
    ...
    <div className="flex flex-col h-screen">
        ...
        <div className="flex items-center gap-4">
          ...
          {serverRestarted && (
            <div className="flex items-center gap-1.5 text-xs font-medium text-amber-400 bg-amber-500/10 border border-amber-500/20 px-2.5 py-1 rounded-lg animate-pulse">
              <AlertCircle size={12} />
              Сервер перезагружен — статус восстановлен
            </div>
          )}
          ...
          {/* Logout */}
          <button
            onClick={async () => { await authApi.operatorLogout().catch(() => {}); window.location.href = '/operator-login'; }}
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
                            ...
                            <tr key={`main-${phone}`} className="hover:bg-accent/30 transition-colors">
                              ...
                              <td className="px-4 py-3">
                                <div className="flex items-center gap-1.5">
                                  <button
                                    title="Перезвонить"
                                    disabled={status !== 'IDLE' || redialMutation.isPending}
                                  ...
                                  {lastCall && (
                                    <button
                                      title="Редактировать"
                                      onClick={() => { setEditCall(lastCall); setEditFormValues(lastCall.formData ? { ...lastCall.formData } : {}); }}
                                ...
                                <td className="px-4 py-2">
                                  {prev.result ? (
                                    <span className={['text-[11px] font-medium px-2 py-0.5 rounded-full', RESULT_COLORS[prev.result] ?? 'text-muted-foreground bg-muted/40'].join(' ')}>
                                      {RESULT_LABELS[prev.result] ?? prev.result}
                                    </span>
                                  ) : <span className="text-[11px] text-muted-foreground">—</span>}
                                </td>
          ...
          {status === 'ACW' && (
            <div className="flex-1 flex flex-col">
              <div className="bg-orange-900/20 border-b border-orange-500/20 px-6 py-3 flex items-center justify-between">
                <div>
                  <span className="text-sm font-medium text-orange-400">ACW — After Call Work</span>
                  <span className="text-xs text-muted-foreground ml-3">Заполните форму до завершения таймера</span>
                </div>
                <div className="flex items-center gap-4">
                  ...
                  <button onClick={saveAndNext}
                    className="px-4 py-1.5 bg-primary hover:bg-[#6b84ff] rounded-lg text-xs font-medium transition-colors">
                    Сохранить и далее
                  </button>
                </div>
              </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm">
          <div className="bg-card border border-border rounded-2xl p-6 w-96 shadow-2xl">
            ...
            <div className="flex flex-col gap-2 max-h-72 overflow-y-auto mb-4">
              {campaigns.filter((c: Campaign) => c.status === 'ACTIVE' && c.dialMode !== 'MANUAL').map((c: Campaign) => (
