              ...
              <Tr key={op.id}>
                <Td>
                  <div className="flex items-center gap-2.5">
                    ...
                    {(op as any).canReceiveInbound && (
                      <span title="Принимает входящие" className="flex items-center gap-1 text-[10px] px-1.5 py-0.5 rounded-full bg-violet-500/10 border border-violet-500/20 text-violet-400 font-medium">
      ...
      <Modal open={canManage && editModal} onClose={() => setEditModal(false)} title="Редактировать оператора">
        <div className="flex flex-col gap-4">
          ...
          <div className="flex items-center justify-between p-3 rounded-lg border border-border bg-muted/20">
            <div>
              <div className="text-sm font-medium text-foreground">Приём входящих звонков</div>
              <div className="text-xs text-muted-foreground mt-0.5">Оператор будет участвовать в маршрутизации входящих</div>
            </div>
            <button
              type="button"
              onClick={() => setEditForm(p => ({ ...p, canReceiveInbound: !p.canReceiveInbound }))}
              className={['relative inline-flex h-6 w-11 rounded-full transition-colors', editForm.canReceiveInbound ? 'bg-primary' : 'bg-muted'].join(' ')}
            >
              <span className={['absolute top-0.5 left-0.5 w-5 h-5 rounded-full bg-white transition-transform shadow', editForm.canReceiveInbound ? 'translate-x-5' : 'translate-x-0'].join(' ')} />
            </button>
          </div>
