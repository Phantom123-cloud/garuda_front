...
function RecordingRow({ rec, isActive, onActivate }: { rec: any; isActive: boolean; onActivate: () => void }) {
  ...
  const call = rec.call;
  // Use backend-resolved custom label first; fall back to hardcoded RESULT_LABELS
  const resultLabel = rec.resultLabel ?? (call.result ? RESULT_LABELS[call.result]?.label : null);
  const resultColor = call.result ? (RESULT_LABELS[call.result]?.color ?? 'text-muted-foreground') : '';
      ...
      <div className="grid grid-cols-[200px_1fr_90px_36px] gap-4 items-start px-4 py-3">
        ...
        <div>
          ...
          <div className="mt-1 flex items-center gap-2">
            ...
            {resultLabel && <span className={`text-[10px] font-medium ${resultColor}`}>{resultLabel}</span>}
          </div>
        </div>
