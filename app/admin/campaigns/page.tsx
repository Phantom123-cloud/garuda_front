...
const EMPTY_FORM = {
  name: '', dialMode: 'PROGRESSIVE', dialOverheadPct: 20, dialTimeout: 25,
      ...
      <Modal open={canManage && showCreate} onClose={() => setShowCreate(false)} title="Новая кампания" width="w-[600px]">
        <div className="flex flex-col gap-5">
            ...
            <div className="grid grid-cols-3 gap-3">
              {DIAL_OPTS.map(o => (
                <button key={o.value}
                  ...
                  onClick={() => !(o as any).disabled && setForm(p => ({ ...p, dialMode: o.value }))}
                  className={['p-3 rounded-xl border text-left transition-all', (o as any).disabled ? 'opacity-40 cursor-not-allowed border-border' : form.dialMode === o.value ? 'border-primary bg-primary/15' : 'border-border hover:border-primary'].join(' ')}>
          ...
          <div className="flex gap-3 pt-1">
            <Button loading={create.isPending} disabled={!form.name.trim() || !form.formId} onClick={() => create.mutate()}>Создать кампанию</Button>
                  ...
                  <div className="flex items-center gap-2">
                    <div>
                      <div className="flex items-center gap-1.5 flex-wrap">
                        ...
                        {/* [INBOUND HIDDEN] isGeneralInbound badge */}
                        {/* [INBOUND HIDDEN] allowInbound badge */}
                      </div>
